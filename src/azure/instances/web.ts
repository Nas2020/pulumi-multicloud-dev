...
import { WebConfig, WebInstanceOutputs } from "./types";

export function createWebInstance(
    instanceConfig: WebConfig,
    ...
    const rawServerName = pulConfig.get("azureWebServerDNS") || "";
    const rawLetsEncryptEmail = pulConfig.get("azureLetsEncryptEmail") || "";
    ...
    // Generate a bootstrap script for nginx configuration
    const bootstrapScript = pulumi.all([instanceConfig.tractionIp, instanceConfig.controllerIp])
        .apply(([tractionIp, controllerIp]) => `#!/bin/bash
set -eo pipefail
...
# For Azure, we need to use a different metadata endpoint
...
if [ -z "$PUBLIC_IP" ]; then
    # Fallback to external service if Azure metadata doesn't work
...
backend_ready=false
for i in {1..30}; do
    if nc -z ${tractionIp} 80 && nc -z ${controllerIp} 80; then
...
cat >> /etc/nginx/sites-available/default <<EOL
    ...
    location /controller/ {
        ...
        proxy_pass http://${controllerIp}:80;
        proxy_http_version 1.1;
        proxy_set_header Host \\\$host;
        proxy_set_header X-Real-IP \\\$remote_addr;
        proxy_set_header X-Forwarded-For \\\$proxy_add_x_forwarded_for;
...
if [ "$USE_SSL" = true ]; then
    ...
    const vm = new compute.VirtualMachine(instanceConfig.name, {
        ...
        osProfile: {
            ...
            customData: bootstrapScript.apply(script =>
