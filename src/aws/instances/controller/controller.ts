...
} from "../traction/user-data-scripts";
import {
    generateControllerAppSetupScript,
    generateControllerFinalizationScript
} from "./controller-user-data-scripts";
...
export function createControllerInstance(
    ...
    const userData = pulumi.interpolate`${generateBaseSetupScript(config.name, publicIp)} 
    ${generateControllerAppSetupScript(currentRegion, secretArn, configBucket, configKey)} 
    ${generateControllerFinalizationScript(config.name)}`;
